...
async def validate_complete_optimization_suite():
    ...
    print(f"  📊 Results: {len(orchestrator_results)} checks, {failed_checks} failed, {cached_checks} cached")
    ...
    for scenario_name, checks, use_cache in scenarios:
        ...
        success_rate = sum(1 for r in scenario_results.values() if r.get("status") == "ok") / len(scenario_results) * 100
        ...
        print(f"  🎯 {scenario_name}: {elapsed:.2f}s ({len(checks)} checks, {success_rate:.1f}% success)")
