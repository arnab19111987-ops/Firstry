...
class BenchmarkRunner:
    ...
    def run_cmd(
        self, cmd: str, cwd: pathlib.Path, cache_state: str = "unknown"
    ) -> Dict[str, Any]:
    ...
    def run_benchmark_suite(self) -> Dict[str, Any]:
        ...
        for repo in self.config["repos"]:
            ...
            print(
                f"🔥 Benchmarking {repo['id']}: {repo.get('description', 'No description')}"
            )
            ...
            for cmd_spec in repo["commands"]:
                ...
                print(
                    f"     {status} {result['total_time_sec']}s (exit {result['exit_code']})"
                )
    ...
    def _validate_performance(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if question == "startup-cost":
                if duration <= targets.get("startup_max_sec", 0.5):
                    validation["checks"].append(
                        f"✅ PASS: {cmd} startup ({duration}s <= 0.5s)"
                    )
                elif duration <= targets.get("startup_warn_sec", 1.0):
                    validation["warnings"].append(
                        f"⚠️ WARN: {cmd} startup ({duration}s > 0.5s)"
                    )
                else:
                    validation["failures"].append(
                        f"❌ FAIL: {cmd} startup ({duration}s > 1.0s)"
                    )
            ...
            # Cold run validation
                ...
                max_time = (
                    targets.get("cold_large_max_sec", 60)
                    if is_large
                    else targets.get("cold_small_max_sec", 30)
                )
                warn_time = targets.get("cold_small_warn_sec", 45)

                if duration <= max_time:
                    validation["checks"].append(
                        f"✅ PASS: {cmd} cold run ({duration}s <= {max_time}s)"
                    )
                elif duration <= warn_time and not is_large:
                    validation["warnings"].append(
                        f"⚠️ WARN: {cmd} cold run ({duration}s > {max_time}s)"
                    )
                else:
                    validation["failures"].append(
                        f"❌ FAIL: {cmd} cold run ({duration}s > {max_time}s)"
                    )
                    validation["overall_pass"] = False
            ...
            # Changed-only validation
                ...
                if duration <= max_time:
                    validation["checks"].append(
                        f"✅ PASS: {cmd} changed-only ({duration}s <= {max_time}s)"
                    )
                else:
                    validation["failures"].append(
                        f"❌ FAIL: {cmd} changed-only ({duration}s > {max_time}s)"
                    )
                    validation["overall_pass"] = False
            ...
            # Strict profile validation
                ...
                if duration <= max_time:
                    validation["checks"].append(
                        f"✅ PASS: {cmd} strict profile ({duration}s <= {max_time}s)"
                    )
                elif duration <= fail_time:
                    validation["warnings"].append(
                        f"⚠️ WARN: {cmd} strict profile ({duration}s > {max_time}s)"
                    )
                else:
                    validation["failures"].append(
                        f"❌ FAIL: {cmd} strict profile ({duration}s > {fail_time}s)"
                    )
                    validation["overall_pass"] = False
    ...
    def _generate_summary(
        self, question_results: Dict[str, List[Dict[str, Any]]]
    ) -> Dict[str, Any]:
    ...
    def print_summary_report(self, results: Dict[str, Any]):
        ...
        if "startup_cost" in summary:
            ...
            print(
                f"1. 🚀 Startup cost: {s['avg_time_sec']}s avg, {s['max_time_sec']}s max ({s['commands_tested']} tests)"
            )

        if "cold_run" in summary:
            s = summary["cold_run"]
            print(
                f"2. ❄️  Cold run: {s['avg_time_sec']}s avg, {s['max_time_sec']}s max ({s['commands_tested']} tests)"
            )

        if "warm_run" in summary:
            s = summary["warm_run"]
            print(
                f"3. 🔥 Warm run: {s['avg_time_sec']}s avg, {s['max_time_sec']}s max ({s['commands_tested']} tests)"
            )

        if "profile_cost" in summary:
            s = summary["profile_cost"]
            print(
                f"4. ⚖️  Profile cost: {s['avg_time_sec']}s avg, {s['max_time_sec']}s max ({s['commands_tested']} tests)"
            )

        if "ci_parity_cost" in summary:
            s = summary["ci_parity_cost"]
            print(
                f"5. 🔄 CI parity: {s['avg_time_sec']}s avg, {s['max_time_sec']}s max ({s['commands_tested']} tests)"
            )
