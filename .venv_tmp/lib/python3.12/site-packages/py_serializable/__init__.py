...
class _SerializableJsonEncoder(JSONEncoder):
    ...
    def __init__(
        ...
        *,
        ...
        view_: Optional[Type[ViewType]] = None,
    ) -> None:
        super().__init__(
            skipkeys=skipkeys,
    ...
    def default(self, o: Any) -> Any:
        ...
        if isinstance(o, object):
            ...
            serializable_property_info = ObjectMetadataLibrary.klass_property_mappings.get(
                klass_qualified_name, {}
            )
                    ...
                    d.update(
                        {
                            new_key: (
                                v
                                if v is not None
                                else prop_info.get_none_value_for_view(view_=self._view)
                            )
                        }
                    )
...
class _JsonSerializable(Protocol):
    ...
    def from_json(cls: Type[_T], data: Dict[str, Any]) -> Optional[_T]:
        ...
        klass_properties = ObjectMetadataLibrary.klass_property_mappings.get(
            klass_qualified_name, {}
        )

        if klass is None:
            _logger.warning(
                "%s is not a known py_serializable class", klass_qualified_name, stacklevel=2
            )
            return None
        ...
        for k, v in data.items():
            ...
            if decoded_k in klass.ignore_during_deserialization:
                _logger.debug(
                    "Ignoring %s when deserializing %s.%s", k, cls.__module__, cls.__qualname__
                )
                continue
            ...
            if new_key is None:
                if klass.ignore_unknown_during_deserialization:
                    ...
                    continue
                _logger.error(
                    "Unexpected key %s/%s in data being serialized to %s.%s",
                    k,
                    decoded_k,
                    cls.__module__,
                    cls.__qualname__,
        ...
        for k, v in _data.items():
            ...
            try:
                if prop_info.custom_type:
                    ...
                    global_klass_name = (
                        f"{prop_info.concrete_type.__module__}.{prop_info.concrete_type.__name__}"
                    )
                ...
                _logger.exception(
                    "There was an AttributeError deserializing JSON to %s.\n"
                    "The Property is: %s\n"
                    "The Value was: %s\n",
                    cls,
                    prop_info,
...
class _XmlSerializable(Protocol):
    ...
    def as_xml(
        ...
        for k, v in self.__dict__.items():
            ...
            if new_key in serializable_property_info:
                prop_info = cast(
                    "ObjectMetadataLibrary.SerializableProperty",
                    serializable_property_info.get(new_key),
                )
        ...
        element_name = _namespace_element_name(
            (
                element_name
                if element_name
                else CurrentFormatter.formatter.encode(self.__class__.__name__)
            ),
            xmlns,
        )
        ...
        for k, prop_info in serializable_property_info.items():
            ...
            if not prop_info.is_xml_attribute:
                ...
                if new_key == ".":
                    this_e.text = _xs_string_mod_apply(str(v), prop_info.xml_string_config)
                    continue
                ...
                if prop_info.is_array and prop_info.xml_array_config:
                        ...
                        if not prop_info.is_primitive_type() and not prop_info.is_enum:
                            nested_e.append(
                                j.as_xml(
                                    view_=view_,
                                    as_string=False,
                                    element_name=nested_key,
                                    xmlns=xmlns,
                                )
                            )
                        elif prop_info.is_enum:
                            SubElement(nested_e, nested_key).text = _xs_string_mod_apply(
                                str(j.value), prop_info.xml_string_config
                            )
                            ...
                            # Assume type is str
                            SubElement(nested_e, nested_key).text = _xs_string_mod_apply(
                                str(j), prop_info.xml_string_config
                            )
                    ...
                    if prop_info.is_helper_type():
                        ...
                        if v_ser is None:
                            ...
                            SubElement(this_e, new_key).text = _xs_string_mod_apply(
                                str(v_ser), prop_info.xml_string_config
                            )
                    else:
                        SubElement(this_e, new_key).text = _xs_string_mod_apply(
                            str(prop_info.custom_type(v)), prop_info.xml_string_config
                        )
                elif prop_info.is_enum:
                    SubElement(this_e, new_key).text = _xs_string_mod_apply(
                        str(v.value), prop_info.xml_string_config
                    )
                elif not prop_info.is_primitive_type():
                    global_klass_name = (
                        f"{prop_info.concrete_type.__module__}.{prop_info.concrete_type.__name__}"
                    ...
                    if global_klass_name in ObjectMetadataLibrary.klass_mappings:
                        ...
                        this_e.append(
                            v.as_xml(
                                view_=view_, as_string=False, element_name=new_key, xmlns=xmlns
                            )
                        )
                    else:
                        ...
                        if prop_info.string_format:
                            SubElement(this_e, new_key).text = _xs_string_mod_apply(
                                f"{v:{prop_info.string_format}}", prop_info.xml_string_config
                            )
                        else:
                            SubElement(this_e, new_key).text = _xs_string_mod_apply(
                                str(v), prop_info.xml_string_config
                            )
                elif prop_info.concrete_type in (float, int):
    ...
    @classmethod
    def from_xml(
        cls: Type[_T], data: Union[TextIOBase, Element], default_namespace: Optional[str] = None
    ) -> Optional[_T]:
        """
        Internal method that is injected into Classes that are annotated for serialization and deserialization by
        ``py_serializable``.
        ...
        if default_namespace is None:
            _namespaces = dict(
                node
                for _, node in SafeElementTree.iterparse(
                    StringIO(SafeElementTree.tostring(data, "unicode")), events=["start-ns"]
                )
            )
            default_namespace = (
                re_compile(r"^\{(.*?)\}.").search(data.tag) or (None, _namespaces.get(""))
            )[1]
        ...
        for k, v in data.attrib.items():
            ...
            if decoded_k in klass.ignore_during_deserialization:
                _logger.debug(
                    "Ignoring %s when deserializing %s.%s",
                    decoded_k,
                    cls.__module__,
                    cls.__qualname__,
                )
            ...
            if not prop_info:
                if klass.ignore_unknown_during_deserialization:
                    ...
                    continue
                raise ValueError(
                    f"Non-primitive types not supported from XML Attributes - see {decoded_k} for "
            ...
            if prop_info.custom_type and prop_info.is_helper_type():
                ...
                raise ValueError(
                    f"Non-primitive types not supported from XML Attributes - see {decoded_k}"
                )
        ...
        for child_e in data:
            ...
            if not prop_info:
                ...
                raise ValueError(
                    f"{decoded_k} is not a known Property for {cls.__module__}.{cls.__qualname__}"
                )
            ...
            try:
                ...
                if prop_info.is_array and prop_info.xml_array_config:
                    ...
                    if array_type == XmlArraySerializationType.NESTED:
                        for sub_child_e in child_e:
                            if sub_child_e.text:
                                sub_child_e.text = _xs_string_mod_apply(
                                    sub_child_e.text, prop_info.xml_string_config
                                )
                            if not prop_info.is_primitive_type() and not prop_info.is_enum:
                                _data[decoded_k].append(
                                    prop_info.concrete_type.from_xml(
                ...
                raise AttributeError(
                    "There was an AttributeError deserializing XML "
                    f"to {cls} the Property {prop_info}: {e}"
                ) from e
...
class ObjectMetadataLibrary:
    ...
    _deferred_property_type_parsing: Dict[
        str, Set["ObjectMetadataLibrary.SerializableProperty"]
    ] = {}
    ...
    class SerializableProperty:
        ...
        def __init__(
            ...
            *,
            prop_name: str,
            prop_type: Any,
            custom_names: Dict[SerializationType, str],
            custom_type: Optional[Any] = None,
            include_none_config: Optional[Set[Tuple[Type[ViewType], Any]]] = None,
            is_xml_attribute: bool = False,
            string_format_: Optional[str] = None,
            ...
            xml_sequence_: Optional[int] = None,
        ) -> None:
        ...
        def _parse_type(self, type_: Any) -> None:
            ...
            if type(type_) is str:
                ...
                match = re_search(
                    r"^(?P<array_type>[\w.]+)\[['\"]?(?P<array_of>\w+)['\"]?]$", type_to_parse
                )
                if match:
                    results = match.groupdict()
                    if results.get("array_type") in self._SORTED_CONTAINERS_TYPES:
                        mapped_array_type = self._SORTED_CONTAINERS_TYPES.get(
                            str(results.get("array_type"))
                        )
                        self._is_array = True
                        try:
        ...
        def __hash__(self) -> int:
            return hash(
                (
                    self.concrete_type,
                    tuple(self.custom_names),
                    self.custom_type,
                    self.is_array,
                    self.is_enum,
                    self.is_optional,
                    self.is_xml_attribute,
                    self.name,
                    self.type_,
                    tuple(self.xml_array_config) if self.xml_array_config else None,
                    self.xml_sequence,
                )
            )
        ...
        def __repr__(self) -> str:
            return (
                f"<s.oml.SerializableProperty name={self.name}, custom_names={self.custom_names}, "
    ...
    def defer_property_type_parsing(
        cls, prop: "ObjectMetadataLibrary.SerializableProperty", klasses: Iterable[str]
    ) -> None:
        for _k in klasses:
    ...
    def register_klass(
        ...
        cls.klass_mappings[f"{klass.__module__}.{klass.__qualname__}"] = (
            ObjectMetadataLibrary.SerializableClass(
                klass=klass,
        ...
        for name, o in getmembers(klass, ObjectMetadataLibrary.is_property):
            ...
            cls.klass_property_mappings[qualified_class_name][name] = (
                ObjectMetadataLibrary.SerializableProperty(
                    prop_name=name,
                    custom_names=ObjectMetadataLibrary._klass_property_names.get(
                        qualified_property_name, {}
                    ),
                    prop_type=prop_arg_specs.annotations.get("return"),
                    custom_type=ObjectMetadataLibrary._klass_property_types.get(
                        qualified_property_name
                    ),
                    ...
                    is_xml_attribute=(
                        qualified_property_name in ObjectMetadataLibrary._klass_property_attributes
                    ),
                    ...
                    xml_string_config=ObjectMetadataLibrary._klass_property_string_config.get(
                        qualified_property_name
                    ),
        ...
        if SerializationType.JSON in serialization_types:
            ...
            klass.from_json = classmethod(
                _JsonSerializable.from_json.__func__
            )  # type:ignore[attr-defined]

        if SerializationType.XML in serialization_types:
            klass.as_xml = _XmlSerializable.as_xml  # type:ignore[attr-defined]
            klass.from_xml = classmethod(
                _XmlSerializable.from_xml.__func__
            )  # type:ignore[attr-defined]
    ...
    @classmethod
    def register_property_include_none(
        cls,
        qual_name: str,
        view_: Optional[Type[ViewType]] = None,
        none_value: Optional[Any] = None,
    ) -> None:
    ...
    @classmethod
    def register_xml_property_array_config(
        cls, qual_name: str, array_type: XmlArraySerializationType, child_name: str
    ) -> None:
        cls._klass_property_array_config[qual_name] = (array_type, child_name)

    @classmethod
    def register_xml_property_string_config(
        cls, qual_name: str, string_type: Optional[XmlStringSerializationType]
    ) -> None:
...
def serializable_enum(
    cls: Optional[Type[_E]] = None,
) -> Union[Callable[[Type[_E]], Type[_E]], Type[_E]]:
    """Decorator"""
...
def serializable_class(
    ...
    *,
    ...
    [Type[_T]], Intersection[Type[_T], Type[_JsonSerializable], Type[_XmlSerializable]]
]: ...
    ...
    *,
    ...
    def decorate(
        ...
        ObjectMetadataLibrary.register_klass(
            klass=kls,
            custom_name=name,
            serialization_types=serialization_types or [],
            ignore_during_deserialization=ignore_during_deserialization,
            ignore_unknown_during_deserialization=ignore_unknown_during_deserialization,
        )
        return kls
...
def include_none(
    view_: Optional[Type[ViewType]] = None, none_value: Optional[Any] = None
) -> Callable[[_F], _F]:
    """Decorator"""

    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s to include None for view: %s", f.__module__, f.__qualname__, view_
        )
...
def string_format(format_: str) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s with String Format: %s", f.__module__, f.__qualname__, format_
        )
...
def xml_attribute() -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        ...
        ObjectMetadataLibrary.register_xml_property_attribute(
...
def xml_array(array_type: XmlArraySerializationType, child_name: str) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s as XML Array: %s:%s",
            f.__module__,
            f.__qualname__,
            array_type,
            child_name,
...
def xml_string(string_type: XmlStringSerializationType) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s as XML StringType: %s", f.__module__, f.__qualname__, string_type
        )
...
def xml_sequence(sequence: int) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s with XML sequence: %s", f.__module__, f.__qualname__, sequence
        )
