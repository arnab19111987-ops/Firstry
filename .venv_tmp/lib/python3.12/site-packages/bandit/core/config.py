...
class BanditConfig:
    ...
    def convert_legacy_config(self):
        ...
        if updated_profiles:
            self.convert_legacy_blacklist_tests(updated_profiles, bad_calls, bad_imports)
    ...
    def convert_names_to_ids(self):
        ...
        for name, profile in (self.get_option("profiles") or {}).items():
            ...
            include = {(extman.get_test_id(i) or i) for i in (profile.get("include") or [])}
            exclude = {(extman.get_test_id(i) or i) for i in (profile.get("exclude") or [])}
    ...
    def convert_legacy_blacklist_data(self):
        ...
        if bad_imports_list or bad_calls_list:
            LOG.warning("Legacy blacklist data found in config, overriding " "data plugins")
