...
from . import _config, _log, version
...
def _find_close_env_var_matches(
    ...
    close_matches = get_close_matches(expected_suffix, candidates, n=3, cutoff=threshold)
...
def _read_pretended_metadata_for(
    ...
    if pretended:
        try:
            ...
            if invalid_fields:
                log.warning(
                    "Invalid metadata fields in pretend metadata: %s. " "Valid fields are: %s",
                    invalid_fields,
                    valid_fields,
                )
