...
from . import _entrypoints, _modify_version
...
from . import _config, _version_cls as _v
...
def _parse_version_tag(tag: str | object, config: _config.Configuration) -> _TagDict | None:
    ...
    if match:
        ...
        log.debug("key %s data %s, %s, %r", key, match.groupdict(), match.groups(), full)
...
def tag_to_version(tag: _VersionT | str, config: _config.Configuration) -> _VersionT | None:
    """
    ...
    try:
        ...
        warnings.warn(f"tag {tag!r} will be stripped of its suffix {tag_dict.get('suffix', '')!r}")
...
def guess_next_date_ver(
    ...
    if match is None:
        warnings.warn(
            f"{version} does not correspond to a valid versioning date, " "assuming legacy version"
        )
        if date_fmt is None:
            date_fmt = "%y.%m.%d"
    else:
    ...
    if match is None:
        ...
        tag_date = (
            datetime.strptime(match.group("date"), date_fmt).replace(tzinfo=timezone.utc).date()
        )
    if tag_date == head_date:
        ...
        if tag_date > head_date and match is not None:
            ...
            warnings.warn(f"your previous tag  ({tag_date}) is ahead your node date ({head_date})")
        patch = 0
...
def _combine_version_with_local_parts(main_version: str, *local_parts: str | None) -> str:
    """
...
def format_version(version: ScmVersion) -> str:
    ...
    return _combine_version_with_local_parts(str(main_version), original_local, local_version)
