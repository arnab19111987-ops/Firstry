...
from . import Configuration, _types as _t, discover
from ._run_cmd import (
    CompletedProcess as _CompletedProcess,
    require_command as _require_command,
    run as _run,
)
from .integration import data_from_mime
from .scm_workdir import Workdir, get_latest_file_mtime
from .version import ScmVersion, meta, tag_to_version
...
class GitWorkdir(Workdir):
    ...
    def is_dirty(self) -> bool:
        return run_git(["status", "--porcelain", "--untracked-files=no"], self.path).parse_success(
            parse=bool,
            default=False,
        )
    ...
    def node(self) -> str | None:
        return run_git(["rev-parse", "--verify", "--quiet", "HEAD"], self.path).parse_success(
            parse=str,
        )
...
def fail_on_shallow(wd: GitWorkdir) -> None:
    """experimental, may change at any time"""
    if wd.is_shallow():
        raise ValueError(f'{wd.path} is shallow, please correct with "git fetch --unshallow"')
...
def _git_parse_inner(
    ...
    if version is None:
        ...
        if node is None:
            ...
            node = "g" + node
            dirty = wd.is_dirty()
        version = meta(tag=tag, distance=distance, dirty=dirty, node=node, config=config)
