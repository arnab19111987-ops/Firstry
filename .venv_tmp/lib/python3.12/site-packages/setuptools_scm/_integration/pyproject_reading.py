...
from .toml import TOML_RESULT, InvalidTomlError, read_toml_content
    ...
    def should_infer(self) -> bool:
        ...
        if self.project_present:
            ...
            if "version" in dynamic_fields:
                if has_build_package_with_extra(self.build_requires, "setuptools-scm", "simple"):
                    return True

        return False


def has_build_package(requires: Sequence[str], canonical_build_package_name: str) -> bool:
...
def read_pyproject(
    ...
    setuptools_dynamic_version = (
        defn.get("tool", {}).get("setuptools", {}).get("dynamic", {}).get("version", None)
    )
    ...
    if _ROOT in kwargs:
        if kwargs[_ROOT] is None:
            ...
            if section[_ROOT] != kwargs[_ROOT]:
                warnings.warn(
                    f"root {section[_ROOT]} is overridden" f" by the cli arg {kwargs[_ROOT]}"
                )
