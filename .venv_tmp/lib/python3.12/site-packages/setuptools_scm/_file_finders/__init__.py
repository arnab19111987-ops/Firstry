...
def scm_find_files(
    ...
    for dirpath, dirnames, filenames in os.walk(realpath, followlinks=True):
        ...
        if not force_all_files and realdirpath not in scm_dirs:
            ...
            dirnames[:] = []
            continue
        if os.path.islink(dirpath) and not os.path.relpath(realdirpath, realpath).startswith(
            os.pardir
        ):
...
def is_toplevel_acceptable(toplevel: str | None) -> TypeGuard[str]:
    ...
    ignored: list[str] = os.environ.get("SETUPTOOLS_SCM_IGNORE_VCS_ROOTS", "").split(os.pathsep)
