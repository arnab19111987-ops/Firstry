...
import textwrap
import warnings
from typing import TYPE_CHECKING, Callable, Final, Mapping, Sequence, TypeVar, overload
...
class CompletedProcess(BaseCompletedProcess):
    @classmethod
    def from_raw(cls, input: BaseCompletedProcess, strip: bool = True) -> CompletedProcess:
        return cls(
            args=input.args,
...
def no_git_env(env: Mapping[str, str]) -> dict[str, str]:
    ...
    for k, v in env.items():
        if k.startswith("GIT_"):
            log.debug("%s: %s", k, v)
    return {k: v for k, v in env.items() if not k.startswith("GIT_") or k in KEEP_GIT_ENV}
...
def avoid_pip_isolation(env: Mapping[str, str]) -> dict[str, str]:
    ...
    new_env["PYTHONPATH"] = os.pathsep.join(
        [path for path in new_env["PYTHONPATH"].split(os.pathsep) if "-build-env-" not in path]
    )
    return new_env
...
def has_command(name: str, args: Sequence[str] = ["version"], warn: bool = True) -> bool:
    try:
        p = run([name, *args], cwd=".")
        if p.returncode != 0:
