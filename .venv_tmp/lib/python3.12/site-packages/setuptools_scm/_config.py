...
from typing import TYPE_CHECKING, Any, Pattern, Protocol

if TYPE_CHECKING:
    from . import git

from . import _log, _types as _t
from ._integration.pyproject_reading import (
    PyProjectData,
    get_args_for_pyproject as _get_args_for_pyproject,
    read_pyproject as _read_pyproject,
)
from ._overrides import read_toml_overrides
from ._version_cls import Version as _Version, _validate_version_cls, _VersionT
...
class ParseFunction(Protocol):
    def __call__(self, root: _t.PathT, *, config: Configuration) -> _t.SCMVERSION | None: ...
...
def _check_absolute_root(root: _t.PathT, relative_to: _t.PathT | None) -> str:
    ...
    if relative_to:
        if (
            ...
            warnings.warn(f"absolute root path '{root}' overrides relative_to '{relative_to}'")
...
class Configuration:
    ...
    git_describe_command: dataclasses.InitVar[_t.CMD_TYPE | None] = _GitDescribeCommandDescriptor()
    ...
    scm: ScmConfiguration = dataclasses.field(default_factory=lambda: ScmConfiguration())
    ...
    @classmethod
    def from_data(cls, relative_to: str | os.PathLike[str], data: dict[str, Any]) -> Configuration:
        """
