...
from .license import (
    ...
    LicenseExpression,
    LicenseRepository,
    _LicenseRepositorySerializationHelper,
)
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple(
            (
                ...
                _ComparableTuple(self.lifecycles),
                self.supplier,
                self.timestamp,
                self.tools,
                self.manufacturer,
...
@serializable.serializable_class(
    ignore_during_deserialization=["$schema", "bom_format", "spec_version"]
)
class Bom:
    """
    ...
    def register_dependency(
        self, target: Dependable, depends_on: Optional[Iterable[Dependable]] = None
    ) -> None:
        ...
        if _d:
            ...
            self._dependencies.add(
                Dependency(
                    ref=target.bom_ref,
                    dependencies=(
                        map(lambda _dep: Dependency(ref=_dep.bom_ref), depends_on)
                        if depends_on
                        else []
                    ),
                )
    ...
    def validate(self) -> bool:
        ...
        for elem in chain(  # type: ignore[assignment]
            [self.metadata],
            (
                self.metadata.component.get_all_nested_components(include_self=True)
                if self.metadata.component
                else []
            ),
            chain.from_iterable(
                c.get_all_nested_components(include_self=True) for c in self.components
            ),
            self.services,
        ):
            if len(elem.licenses) > 1 and any(
                isinstance(li, LicenseExpression) for li in elem.licenses
            ):
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple(
            (
                ...
                _ComparableTuple(self.services),
