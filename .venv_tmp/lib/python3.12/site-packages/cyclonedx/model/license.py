...
class _LicenseRepositorySerializationHelper(serializable.helpers.BaseHelper):
    ...
    @classmethod
    def json_normalize(
        cls, o: LicenseRepository, *, view: Optional[Type[serializable.ViewType]], **__: Any
    ) -> Any:
        if len(o) == 0:
            return None
    ...
    @classmethod
    def json_denormalize(cls, o: List[Dict[str, Any]], **__: Any) -> LicenseRepository:
        repo = LicenseRepository()
        for li in o:
            if "license" in li:
                repo.add(
                    DisjunctiveLicense.from_json(  # type:ignore[attr-defined]
                        li["license"]
                    )
                )
            elif "expression" in li:
                repo.add(
                    LicenseExpression.from_json(  # type:ignore[attr-defined]
                        li
                    )
                )
            else:
    ...
    def xml_normalize(
        ...
        **__: Any,
        ...
        if expression:
            ...
            # but models need to allow it for backwards compatibility with JSON CDX < 1.5
            elem.append(
                expression.as_xml(  # type:ignore[attr-defined]
    ...
    @classmethod
    def xml_denormalize(cls, o: Element, default_ns: Optional[str], **__: Any) -> LicenseRepository:
        repo = LicenseRepository()
        for li in o:
            tag = li.tag if default_ns is None else li.tag.replace(f"{{{default_ns}}}", "")
            if tag == "license":
                repo.add(
                    DisjunctiveLicense.from_xml(  # type:ignore[attr-defined]
                        li, default_ns
                    )
                )
            elif tag == "expression":
                repo.add(
                    LicenseExpression.from_xml(  # type:ignore[attr-defined]
                        li, default_ns
                    )
                )
            else:
