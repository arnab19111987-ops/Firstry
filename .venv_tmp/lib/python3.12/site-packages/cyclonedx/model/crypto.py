...
class AlgorithmProperties:
    ...
    def execution_environment(
        self, execution_environment: Optional[CryptoExecutionEnvironment]
    ) -> None:
    ...
    def implementation_platform(
        self, implementation_platform: Optional[CryptoImplementationPlatform]
    ) -> None:
    ...
    @serializable.xml_array(
        serializable.XmlArraySerializationType.FLAT, child_name="certificationLevel"
    )
    ...
    def certification_levels(
        self, certification_levels: Iterable[CryptoCertificationLevel]
    ) -> None:
    ...
    @property
    @serializable.xml_array(
        serializable.XmlArraySerializationType.NESTED, child_name="cryptoFunction"
    )
    ...
    def nist_quantum_security_level(self, nist_quantum_security_level: Optional[int]) -> None:
        if nist_quantum_security_level is not None and (
            nist_quantum_security_level < 0 or nist_quantum_security_level > 6
        ):
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple(
            (
                self.primitive,
                self._parameter_set_identifier,
                self.curve,
                self.execution_environment,
                self.implementation_platform,
                _ComparableTuple(self.certification_levels),
                self.mode,
                self.padding,
                _ComparableTuple(self.crypto_functions),
...
class RelatedCryptoMaterialSecuredBy:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.mechanism, self.algorithm_ref))
...
class RelatedCryptoMaterialProperties:
    ...
    def __repr__(self) -> str:
        return (
            f"<RelatedCryptoMaterialProperties type={self.type}, id={self.id} state={self.state}>"
        )
...
class CryptoProperties:
    ...
    def certificate_properties(
        self, certificate_properties: Optional[CertificateProperties]
    ) -> None:
