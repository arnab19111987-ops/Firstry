...
class DataClassification:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.flow, self.classification))
...
class _HashTypeRepositorySerializationHelper(serializable.helpers.BaseHelper):
    ...
    @classmethod
    def __prep(
        cls, hts: Iterable["HashType"], view: Type[serializable.ViewType]
    ) -> Generator["HashType", None, None]:
        ...
        for ht in hts:
            if ht.alg in cases:
                ...
                warn(
                    f"serialization omitted due to unsupported HashAlgorithm: {ht!r}",
    ...
    @classmethod
    def json_normalize(
        cls, o: Iterable["HashType"], *, view: Optional[Type[serializable.ViewType]], **__: Any
    ) -> List[Any]:
    ...
    @classmethod
    def json_denormalize(cls, o: Any, **__: Any) -> List["HashType"]:
        return [
    ...
    @classmethod
    def xml_denormalize(
        cls, o: "XmlElement", *, default_ns: Optional[str], **__: Any
    ) -> List["HashType"]:
        return [
...
class HashType:
    ...
    def from_composite_str(composite_hash: str) -> "HashType":
        ...
        if algorithm_prefix == "md5":
            return HashType(alg=HashAlgorithm.MD5, content=parts[1].lower())
        elif algorithm_prefix[0:4] == "sha3":
            return HashType(
                alg=getattr(HashAlgorithm, f"SHA3_{algorithm_prefix[5:]}"), content=parts[1].lower()
            )
        elif algorithm_prefix == "sha1":
            return HashType(alg=HashAlgorithm.SHA_1, content=parts[1].lower())
        elif algorithm_prefix[0:3] == "sha":
            # This is actually SHA2...
            return HashType(
                alg=getattr(HashAlgorithm, f"SHA_{algorithm_prefix[3:]}"), content=parts[1].lower()
            )
            ...
            return HashType(alg=HashAlgorithm.BLAKE3, content=parts[1].lower())
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.alg, self.content))
...
class _ExternalReferenceSerializationHelper(serializable.helpers.BaseHelper):
    ...
    @classmethod
    def json_normalize(
        cls, o: Any, *, view: Optional[Type[serializable.ViewType]], **__: Any
    ) -> str:
        assert view is not None
...
class XsUri(serializable.helpers.BaseHelper):
    ...
    def __init__(self, uri: str) -> None:
        if re.search(XsUri._INVALID_URI_REGEX, uri):
            raise InvalidUriException(f"Supplied value '{uri}' does not appear to be a valid URI.")
    ...
    def serialize(cls, o: Any) -> str:
        if isinstance(o, XsUri):
            return str(o)
        raise SerializationOfUnexpectedValueException(f"Attempt to serialize a non-XsUri: {o!r}")
...
class ExternalReference:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple(
            (self._type, self._url, self._comment, _ComparableTuple(self._hashes))
        )
...
class Property:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.name, self.value))
...
class NoteText:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.content, self.content_type, self.encoding))
...
class Note:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.locale, self.text))
...
class IdentifiableAction:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.timestamp, self.name, self.email))
