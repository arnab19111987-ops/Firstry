...
class NamedLifecycle:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self._name, self._description))
...
class _LifecycleRepositoryHelper(BaseHelper):
    @classmethod
    def json_normalize(
        cls, o: LifecycleRepository, *, view: Optional[Type["ViewType"]], **__: Any
    ) -> Any:
        if len(o) == 0:
            return None
        return [
            json_loads(
                li.as_json(  # type:ignore[union-attr]
                    view_=view
                )
            )
            for li in o
    ...
    @classmethod
    def json_denormalize(cls, o: List[Dict[str, Any]], **__: Any) -> LifecycleRepository:
        repo = LifecycleRepository()
        for li in o:
            if "phase" in li:
                repo.add(
                    PredefinedLifecycle.from_json(  # type:ignore[attr-defined]
                        li
                    )
                )
            elif "name" in li:
                repo.add(
                    NamedLifecycle.from_json(  # type:ignore[attr-defined]
                        li
                    )
                )
            else:
    ...
    def xml_normalize(
        ...
        **__: Any,
        ...
        for li in o:
            elem.append(
                li.as_xml(  # type:ignore[union-attr]
    ...
    @classmethod
    def xml_denormalize(
        cls, o: Element, default_ns: Optional[str], **__: Any
    ) -> LifecycleRepository:
        repo = LifecycleRepository()
        ...
        for li in o.iterfind("bom:lifecycle", ns_map):
            if li.find("bom:phase", ns_map) is not None:
                repo.add(
                    PredefinedLifecycle.from_xml(  # type:ignore[attr-defined]
                        li, default_ns
                    )
                )
            elif li.find("bom:name", ns_map) is not None:
                repo.add(
                    NamedLifecycle.from_xml(  # type:ignore[attr-defined]
                        li, default_ns
                    )
                )
            else:
