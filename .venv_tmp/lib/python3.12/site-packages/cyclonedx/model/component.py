...
from .._internal.compare import (
    ComparablePackageURL as _ComparablePackageURL,
    ComparableTuple as _ComparableTuple,
)
...
class Commit:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.uid, self.url, self.author, self.committer, self.message))
...
class _ComponentScopeSerializationHelper(serializable.helpers.BaseHelper):
    ...
    def __normalize(cls, cs: ComponentScope, view: Type[serializable.ViewType]) -> Optional[str]:
        return cs.value if cs in cls.__CASES.get(view, ()) else None
...
class _ComponentTypeSerializationHelper(serializable.helpers.BaseHelper):
    ...
    def __normalize(cls, ct: ComponentType, view: Type[serializable.ViewType]) -> Optional[str]:
        ...
        raise SerializationOfUnsupportedComponentTypeException(
            f"unsupported {ct!r} for view {view!r}"
        )
...
class Patch:
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple((self.type, self.diff, _ComparableTuple(self.resolves)))
...
class Swhid(serializable.helpers.BaseHelper):
    ...
    def __init__(self, id: str) -> None:
        if Swhid._VALID_SWHID_REGEX.match(id) is None:
            raise InvalidSwhidException(f'Supplied value "{id} does not meet format specification.')
        self._id = id
    ...
    def serialize(cls, o: Any) -> str:
        if isinstance(o, Swhid):
            return str(o)
        raise SerializationOfUnexpectedValueException(f"Attempt to serialize a non-Swhid: {o!r}")
...
class Component(Dependable):
    ...
    def for_file(absolute_file_path: str, path_for_bom: Optional[str]) -> "Component":
        ...
        return Component(
            ...
            hashes=[HashType(alg=HashAlgorithm.SHA_1, content=sha1_hash)],
    ...
    def __init__(
        ...
        *,
        ...
        if modified:
            warn(
                "`.component.modified` is deprecated from CycloneDX v1.3 onwards. "
                "Please use `@.pedigree` instead.",
                DeprecationWarning,
            )
        if author:
            warn(
    ...
    def version(self, version: Optional[str]) -> None:
        if version and len(version) > 1024:
            warn(
                "`.component.version`has a maximum length of 1024 from CycloneDX v1.6 onwards.",
                UserWarning,
            )
        self._version = version
    ...
    def __comparable_tuple(self) -> _ComparableTuple:
        return _ComparableTuple(
            (
                ...
                self.mime_type,
                self.scope,
                _ComparableTuple(self.hashes),
                _ComparableTuple(self.licenses),
                self.copyright,
                self.pedigree,
                _ComparableTuple(self.external_references),
                _ComparableTuple(self.properties),
                _ComparableTuple(self.components),
                self.evidence,
                self.release_notes,
                self.modified,
                _ComparableTuple(self.authors),
                _ComparableTuple(self.omnibor_ids),
                self.manufacturer,
    ...
    def __repr__(self) -> str:
        return (
            f"<Component bom-ref={self.bom_ref!r}, group={self.group}, name={self.name}, "
