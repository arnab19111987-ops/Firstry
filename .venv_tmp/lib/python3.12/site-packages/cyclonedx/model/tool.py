...
class ToolRepository:
    ...
    def __init__(
        ...
        *,
        ...
        if tools:
            warn(
                "`@.tools` is deprecated from CycloneDX v1.5 onwards. "
...
class _ToolRepositoryHelper(BaseHelper):
    ...
    @classmethod
    def json_normalize(
        cls, o: ToolRepository, *, view: Optional[Type["ViewType"]], **__: Any
    ) -> Any:
    ...
    def json_denormalize(
        cls, o: Union[List[Dict[str, Any]], Dict[str, Any]], **__: Any
    ) -> ToolRepository:
        tools = None
        components = None
        services = None
        if isinstance(o, Dict):
            components = map(
                lambda c: Component.from_json(  # type:ignore[attr-defined]
                    c
                ),
                o.get("components", ()),
            )
            services = map(
                lambda s: Service.from_json(  # type:ignore[attr-defined]
                    s
                ),
                o.get("services", ()),
            )
            ...
            tools = map(
                lambda t: Tool.from_json(  # type:ignore[attr-defined]
                    t
                ),
                o,
            )
        ...
        **__: Any,
        ...
        if len(o.tools) > 0 or not cls.__supports_components_and_services(view):
            ...
            if o.services:
                ...
                elem.append(elem_s)
        return elem if len(elem) > 0 else None
    ...
    def xml_denormalize(
        ...
        **kwargs: Any,
        ...
        if len(ts) > 0:
            tools = map(
                lambda t: Tool.from_xml(  # type:ignore[attr-defined]
                    t, default_ns
                ),
                ts,
            )
            ...
            components = map(
                lambda c: Component.from_xml(  # type:ignore[attr-defined]
                    c, default_ns
                ),
                o.iterfind("./bom:components/bom:component", ns_map),
            )
            services = map(
                lambda s: Service.from_xml(  # type:ignore[attr-defined]
                    s, default_ns
                ),
                o.iterfind("./bom:services/bom:service", ns_map),
            )
        return ToolRepository(components=components, services=services, tools=tools)
