...
class _BaseJsonValidator(BaseSchemabasedValidator, ABC):
    ...
    if _missing_deps_error:  # noqa:C901
        ...
        def validate_str(self, data: str) -> Optional[ValidationError]:
            return self._validata_data(json_loads(data))
        ...
        def __make_validator_registry() -> Registry[Any]:
            ...
            with open(_S_SPDX) as spdx, open(_S_JSF) as jsf:
                return Registry().with_resources(
                    [
                        (
                            f"{schema_prefix}spdx.SNAPSHOT.schema.json",
                            DRAFT7.create_resource(json_loads(spdx.read())),
                        ),
                        (
                            f"{schema_prefix}jsf-0.82.SNAPSHOT.schema.json",
                            DRAFT7.create_resource(json_loads(jsf.read())),
                        ),
                    ]
                )
