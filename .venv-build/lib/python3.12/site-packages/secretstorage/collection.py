...
from secretstorage.util import (DBusAddressWrapper, exec_prompt, format_secret,
                                open_session, unlock_objects)
...
class Collection:
    ...
    def create_item(
        ...
        item_path, prompt = self._collection.call(
            "CreateItem", "a{sv}(oayays)b", properties, _secret, replace
        )
        if len(item_path) > 1:
...
def create_collection(
    ...
    collection_path, prompt = service.call(
...
def get_default_collection(
    connection: DBusConnection, session: Session | None = None
) -> Collection:
...
def search_items(
    connection: DBusConnection, attributes: dict[str, str]
) -> Iterator[Item]:
