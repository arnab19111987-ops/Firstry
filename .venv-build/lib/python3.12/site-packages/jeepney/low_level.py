...
class FileDescriptor(FixedType):
    ...
    def serialise(self, data, pos, endianness, fds=None):
        ...
        if isinstance(data, bool) or not isinstance(data, int):
            raise TypeError(
                "Cannot use {data!r} as file descriptor. Expected "
...
class Struct:
    ...
    def serialise(self, data, pos, endianness, fds=None):
        ...
        if len(data) != len(self.fields):
            raise ValueError(
                "{} entries for {} fields".format(len(data), len(self.fields))
            )
        pad = b"\0" * padding(pos, self.alignment)
        pos += len(pad)
        res_pieces = []
...
class DictEntry(Struct):
    def __init__(self, fields):
        ...
        if not isinstance(fields[0], (FixedType, StringType)):
            raise TypeError(
                "First field in dict entry must be simple type, not {}".format(
                    type(fields[0])
                )
            )
...
class Array:
    ...
    def serialise(self, data, pos, endianness, fds=None):
        ...
        if data_is_bytes:
            ...
            for item in data:
                chunks.append(
                    self.elt_type.serialise(item, data_pos, endianness, fds=fds)
                )
