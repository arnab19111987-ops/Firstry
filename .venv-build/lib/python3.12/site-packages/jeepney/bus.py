...
def get_connectable_addresses(addr):
    ...
    if not found:
        raise RuntimeError(
            "DBus transports ({}) not supported. Supported: {}".format(
                unsupported_transports, SUPPORTED_TRANSPORTS
            )
        )
...
def find_system_bus():
    addr = (
        os.environ.get("DBUS_SYSTEM_BUS_ADDRESS", "") or "unix:path=/var/run/dbus/system_bus_socket"
    )
    return next(get_connectable_addresses(addr))
