...
def test_match_rule_arg():
    ...
    assert rule.matches(
        new_method_call(portal_req_iface, "Boo", signature="s", body=("foo",))
    )

    assert not rule.matches(
        new_method_call(portal_req_iface, "Boo", signature="s", body=("foobar",))
    )
...
def test_match_rule_arg_path():
    ...
    # Exact match
    assert rule.matches(
        new_method_call(portal_req_iface, "Boo", signature="s", body=("/aa/bb/",))
    )

    # Match a prefix
    assert rule.matches(
        new_method_call(portal_req_iface, "Boo", signature="s", body=("/aa/bb/cc",))
    )

    # Argument is a prefix, ending with /
    assert rule.matches(
        new_method_call(portal_req_iface, "Boo", signature="s", body=("/aa/",))
    )
...
def test_match_rule_arg_namespace():
    ...
    # Exact match
    assert rule.matches(
        new_signal(message_bus, "NameOwnerChanged", "s", ("com.example.backend1",))
    )
    ...
    assert not rule.matches(
        new_signal(message_bus, "NameOwnerChanged", "s", ("com.example.backend12",))
    )
