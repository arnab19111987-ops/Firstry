...
class DistributionPoint:
    def __init__(
        ...
        if full_name and relative_name:
            raise ValueError(
                "You cannot provide both full_name and relative_name, at " "least one must be None."
            )
        ...
        if reasons and (
            ...
            raise ValueError(
                "unspecified and remove_from_crl are not valid reasons in a " "DistributionPoint"
            )
...
class NoticeReference:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self.organization == other.organization and self.notice_numbers == other.notice_numbers
        )
...
class KeyUsage(ExtensionType):
    ...
    def __init__(
        ...
        if not key_agreement and (encipher_only or decipher_only):
            raise ValueError(
                "encipher_only and decipher_only can only be true when " "key_agreement is true"
            )
...
class PrivateKeyUsagePeriod(ExtensionType):
    ...
    def __init__(
        ...
        if not_before is None and not_after is None:
            raise ValueError("At least one of not_before and not_after must not be None")
    ...
    def __repr__(self) -> str:
        return (
            f"<PrivateKeyUsagePeriod(not_before={self.not_before}, " f"not_after={self.not_after})>"
        )
...
class NameConstraints(ExtensionType):
    ...
    def __init__(
        ...
        if permitted_subtrees is not None:
            ...
            if not all(isinstance(x, GeneralName) for x in permitted_subtrees):
                raise TypeError(
                    "permitted_subtrees must be a list of GeneralName objects " "or None"
                )
        ...
        if permitted_subtrees is None and excluded_subtrees is None:
            raise ValueError(
                "At least one of permitted_subtrees and excluded_subtrees " "must not be None"
            )
    ...
    def _validate_ip_name(self, tree: Iterable[GeneralName]) -> None:
        if any(
            ...
            and not isinstance(name.value, (ipaddress.IPv4Network, ipaddress.IPv6Network))
            for name in tree
        ):
            raise TypeError(
                "IPAddress name constraints must be an IPv4Network or" " IPv6Network object"
            )
    ...
    def _validate_dns_name(self, tree: Iterable[GeneralName]) -> None:
        if any(isinstance(name, DNSName) and "*" in name.value for name in tree):
            raise ValueError(
                "DNSName name constraints must not contain the '*' wildcard" " character"
            )
...
class Extension(typing.Generic[ExtensionTypeVar]):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self.oid == other.oid and self.critical == other.critical and self.value == other.value
        )
...
class PrecertificateSignedCertificateTimestamps(ExtensionType):
    ...
    def __init__(
        ...
        if not all(
            isinstance(sct, SignedCertificateTimestamp) for sct in signed_certificate_timestamps
        ):
            raise TypeError(
    ...
    def __init__(
        ...
        if profession_oids is not None:
            ...
            if not all(isinstance(oid, ObjectIdentifier) for oid in profession_oids):
                raise TypeError(
                    "Every item in the profession_oids list must be an " "ObjectIdentifier"
                )
