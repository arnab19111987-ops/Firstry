...
def role(role_name, language_module, lineno, reporter):
    ...
    if role_name:
        ...
        try:
            ...
            msg_text.append(
                'No role entry for "%s" in module "%s".' % (role_name, language_module.__name__)
            )
    else:
...
def pep_reference_role(role, rawtext, text, lineno, inliner, options=None, content=None):
    ...
    try:
        ...
        msg = inliner.reporter.error(
            'PEP number must be a number from 0 to 9999; "%s" is invalid.' % text,
            line=lineno,
        )
...
def rfc_reference_role(role, rawtext, text, lineno, inliner, options=None, content=None):
    ...
    try:
        ...
        msg = inliner.reporter.error(
            "RFC number must be a number greater than or equal to 1; " '"%s" is invalid.' % text,
            line=lineno,
        )
        prb = inliner.problematic(rawtext, rawtext, msg)
        return [prb], [msg]
...
def code_role(role, rawtext, text, lineno, inliner, options=None, content=None):
    options = normalize_options(options)
...
def set_classes(options) -> None:
    ...
    warnings.warn(
        "The auxiliary function roles.set_classes() is obsoleted"
...
def normalized_role_options(options):
    warnings.warn(
        "The auxiliary function roles.normalized_role_options() is "
