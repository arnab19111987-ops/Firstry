...
class Include(Directive):
    ...
    def insert_into_input_lines(self, text: str) -> None:
        ...
        """
        source = self.options["source"]
        textlines = statemachine.string2lines(text, self.tab_width, convert_whitespace=True)
        # Sanity checks:
        # excessively long lines
        for i, line in enumerate(textlines):
            if len(line) > self.settings.line_length_limit:
                line_no = i + 1 + self.options.get("start-line", 0)
                raise self.warning(f'"{source}": line {line_no} exceeds the' " line-length-limit.")
        # circular inclusion
        include_log = self.state.document.include_log
        if not include_log:  # new document, initialize with document source
            current_source = utils.relative_path(None, self.state.document.current_source)
    ...
    def run(self):
        ...
        if self.content:
            ...
            try:
                ...
                raise self.severe(
                    f'Problems with "{self.name}" directive ' f"path:\n{io.error_string(error)}."
                )
            else:
...
class Role(Directive):
    ...
    def run(self):
        ...
        if not match:
            raise self.error(
                '"%s" directive arguments not valid role names: ' '"%s".' % (self.name, args)
            )
