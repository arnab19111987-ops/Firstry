...
class BasePseudoSection(Directive):
    ...
    def run(self):
        if not isinstance(self.state_machine.node, (nodes.document, nodes.section, nodes.sidebar)):
            raise self.error(
                'The "%s" directive may not be used within ' "topics or body elements." % self.name
            )
...
class Sidebar(BasePseudoSection):
    ...
    option_spec = BasePseudoSection.option_spec | {"subtitle": directives.unchanged_required}

    def run(self):
        if isinstance(self.state_machine.node, nodes.sidebar):
            raise self.error(
                'The "%s" directive may not be used within a ' "sidebar element." % self.name
            )
