...
class RSTStateMachine(StateMachineWS):
    ...
    def run(self, input_lines, document, input_offset=0, match_titles=True, inliner=None) -> None:
        """
...
class RSTState(StateWS):
    ...
    def nested_parse(
        ...
        if match_titles:
            ...
            l_start = min(len(self.parent.section_hierarchy()), len(self.memo.title_styles))
    ...
    def check_subsection(self, source, style, lineno) -> bool:
        ...
        if newlevel > oldlevel + 1:
            ...
            self.parent += self.reporter.error(
                "Inconsistent title style:" f" skip from level {oldlevel} to {newlevel}.",
...
class Inliner:
    ...
    def init_customizations(self, settings) -> None:
        ...
        self.patterns = Struct(
            ...
            emphasis=re.compile(self.non_whitespace_escape_before + r"(\*)" + end_string_suffix),
            strong=re.compile(self.non_whitespace_escape_before + r"(\*\*)" + end_string_suffix),
            ...
            target=re.compile(self.non_whitespace_escape_before + r"(`)" + end_string_suffix),
    ...
    def parse(self, text, lineno, memo, parent):
        ...
        while remaining:
            ...
            if match:
                ...
                method = dispatch[
                    groups["start"] or groups["backquote"] or groups["refend"] or groups["fnend"]
                ]
...
class Body(RSTState):
    ...
    def enumerator(self, match, context, next_state):
        ...
        if ordinal != 1:
            ...
            msg = self.reporter.info(
                'Enumerated list start value not ordinal-1: "%s" (ordinal %s)' % (text, ordinal),
                base_node=enumlist,
            )
            self.parent += msg
    ...
    def parse_enumerator(self, match, expected_sequence=None):
        ...
        text = groupdict[format][
            self.enum.formatinfo[format].start : self.enum.formatinfo[format].end  # noqa: E203,E501
        ]
        if text == "#":
            sequence = "#"
    ...
    def run_directive(self, directive, match, type_name, option_presets):
        ...
        block_text = "\n".join(
            self.state_machine.input_lines[initial_line_offset : self.state_machine.line_offset + 1]
        )  # noqa: E203,E501
        try:
...
class Text(RSTState):
    ...
    def definition_list_item(self, termline):
        ...
        (dl_item.source, dl_item.line) = self.state_machine.get_source_and_line(dt_lineno)
        ...
        for i in range(len(text_nodes)):
            ...
            if isinstance(node, nodes.Text):
                ...
                if len(parts) == 1:
                    ...
                    node_list += [
                        nodes.classifier(unescape(part, True), part) for part in parts[1:]
                    ]
            else:
                node_list[-1] += node
