...
class Input(TransformSpec):
    ...
    def determine_encoding_from_data(self, data: bytes) -> str | None:
        ...
        """
        warnings.warn(
            "docutils.io.Input.determine_encoding_from_data() " "will be removed in Docutils 1.0.",
            DeprecationWarning,
            stacklevel=2,
        )
...
class ErrorOutput:
    ...
    def __init__(
        ...
        self.encoding: str = (
            encoding or getattr(destination, "encoding", None) or _locale_encoding or "ascii"
        )
...
class FileOutput(Output):
    ...
    def write(self, data: str | bytes) -> str | bytes:
        ...
        if not self.opened:
            self.open()
        if isinstance(data, str) and check_encoding(self.destination, self.encoding) is False:
            if os.linesep != "\n":
