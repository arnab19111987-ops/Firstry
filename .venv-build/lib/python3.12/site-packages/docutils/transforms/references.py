...
class AnonymousHyperlinks(Transform):
    ...
    def apply(self) -> None:
        anonymous_refs = [
            node for node in self.document.findall(nodes.reference) if node.get("anonymous")
        ]
        anonymous_targets = [
            node for node in self.document.findall(nodes.target) if node.get("anonymous")
        ]
...
class DanglingReferencesVisitor(nodes.SparseNodeVisitor):
    ...
    def visit_reference(self, node) -> None:
        ...
        if refname in self.document.nameids:
            msg = self.document.reporter.error(
                "Duplicate target name, cannot be used as a unique " f'reference: "{refname}".',
                base_node=node,
            )
        else:
