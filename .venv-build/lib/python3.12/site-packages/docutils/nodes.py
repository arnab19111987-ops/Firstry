...
class Element(Node):
    ...
    def __delitem__(self, key: str | int | slice) -> None:
        if isinstance(key, str):
            ...
            raise TypeError(
                "element index must be an integer, a simple " "slice, or an attribute name string"
            )
    ...
    def _report_child(
        ...
        if isinstance(child, Text):
            return (
                f"{msg}  Expecting child of type <{_type}>, " f'not text data "{child.astext()}".'
            )
...
class NodeVisitor:
    ...
    def unknown_visit(self, node) -> None:
        ...
        if self.document.settings.strict_visitor or node.__class__.__name__ not in self.optional:
            raise NotImplementedError(
                "%s visiting unknown node type: %s" % (self.__class__, node.__class__.__name__)
            )
    ...
    def unknown_departure(self, node) -> None:
        ...
        if self.document.settings.strict_visitor or node.__class__.__name__ not in self.optional:
            raise NotImplementedError(
                "%s departing unknown node type: %s" % (self.__class__, node.__class__.__name__)
            )
