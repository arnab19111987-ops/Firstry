...
class SortableDict(dict):  # NoQA: FURB189
    ...
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "`writers.latex2e.SortableDict` is obsolete" " and will be removed in Docutils 0.24.",
            DeprecationWarning,
            stacklevel=2,
        )
...
class Table:
    ...
    def get_colspecs(self, node):
        ...
        if self.colwidths_auto:
            ...
            self._colwidths = [
                (factor * (node.propwidth() + 1) / width) + 0.005 for node in self._col_specs
            ]
...
class LaTeXTranslator(writers.DoctreeTranslator):
    ...
    def __init__(self, document, babel_class=Babel) -> None:
        ...
        if settings.graphicx_option == "":
            ...
            self.graphicx_package = r"\usepackage[%s]{graphicx}" % settings.graphicx_option
    ...
    def visit_entry(self, node):
        ...
        if len(node) and (
            isinstance(node.parent.parent, nodes.thead) or self.active_table.is_stub_column()
        ):
            self.out.append("\\textbf{")
            self.context.append("}")
        else:
    ...
    def visit_contents(self, node):
        ...
        if "local" in node["classes"]:
            ...
            self.requirements["minitoc-%s-depth" % mtc_name] = r"\mtcsetdepth{%stoc}{%d}" % (
                mtc_name,
                maxdepth,
            )
            ...
            if depth:
                self.out.append(
                    "\\setcounter{%stocdepth}{%d}" % (mtc_name, depth + offset[mtc_name])
                )
            # title:
