...
class HTMLTranslator(writers.DoctreeTranslator):
    ...
    def __init__(self, document) -> None:
        ...
        self.language = languages.get_language(settings.language_code, document.reporter)
        ...
        if isinstance(self.settings.math_output, str):
            self.settings.math_output = frontend.validate_math_output(self.settings.math_output)
        ...
        if settings.xml_declaration:
            ...
            self.html_prolog.append(utils.xml_declaration("%s"))  # encoding not interpolated
    ...
    def prepare_svg(self, code: str, node: nodes.Element, atts: dict) -> str:
        ...
        if "style" in atts:
            ...
            svg.set("style", " ".join(f"{k}: {v};" for k, v in declaration_dict.items()))
    ...
    def visit_docinfo_item(self, node, name, meta=True) -> None:
        if meta:
            self.meta.append(f'<meta name="{name}" ' f'content="{self.attval(node.astext())}" />\n')
        self.body.append(
            f'<dt class="{name}">{self.language.labels[name]}' '<span class="colon">:</span></dt>\n'
        )
    ...
    def visit_footnote(self, node) -> None:
        ...
        self.body.append(
            self.starttag(node, "aside", classes=[node.tagname, label_style], role="doc-footnote")
        )
    ...
    def visit_footnote_reference(self, node) -> None:
        ...
        self.body.append(
            self.starttag(node, "a", suffix="", classes=classes, role="doc-noteref", href=href)
        )
    ...
    def visit_image(self, node) -> None:
        ...
        if mimetype in self.videotypes:
            ...
            atts["type"] = mimetype
            element = (
                self.starttag(node, "object", "", data=uri, **atts) + f"{alt}</object>{suffix}"
            )
    ...
    def visit_math(self, node):
        ...
        if format == "html":
            ...
            if not self.math_header:
                if self.math_options:
                    ...
                    self.document.reporter.warning(
                        "No MathJax URL specified, using local fallback " "(see config.html).",
                        base_node=node,
                    )
            ...
            try:
                ...
                self.messages.append(self.document.reporter.warning(err, *details, base_node=node))
        ...
        if tag:
            self.body.append(
                self.starttag(node, tag, suffix=suffix, classes=self.math_tags[format][2])
            )
    ...
    def depart_paragraph(self, node) -> None:
        self.body.append("</p>")
        if not (
            isinstance(node.parent, (nodes.list_item, nodes.entry)) and (len(node.parent) == 1)
        ):
            self.body.append("\n")
            self.report_messages(node)
