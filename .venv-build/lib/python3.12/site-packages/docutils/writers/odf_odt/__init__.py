            ...
            for node in n2_nodes:
                ...
                if family == "paragraph" or family == "graphic":
                    ...
                    if region_code is not None:
                        ...
                        n3.attrib[
                            "{urn:oasis:names:tc:opendocument:xmlns:" "style:1.0}country-complex"
                        ] = region_code
                        updated = True
                        modified_nodes.add(n3)
...
class ODFTranslator(nodes.GenericNodeVisitor):
    ...
    def get_image_width_height(self, node, attr):
        ...
        if attr in node.attributes:
            ...
            try:
                ...
                self.document.reporter.warning(
                    f'Invalid {attr} for image: "{node.attributes[attr]}".  ' f'Error: "{exp}".',
                    base_node=node,
                )
    ...
    def get_page_width(self):
        ...
        for node in nodes:
            ...
            margin_right = node.get(
                "{urn:oasis:names:tc:opendocument:xmlns:xsl-fo-compatible:1.0}" "margin-right"
            )
    ...
    def _add_syntax_highlighting(self, insource, language):
        ...
        if language in ("latex", "tex"):
            fmtr = OdtPygmentsLaTeXFormatter(self.rststyle, escape_function=escape_cdata)
        else:
    ...
    def get_table_style(self, node):
        ...
        if table_name is not None:
            ...
            if table_style is None:
                ...
                #   and use a built-in default style.
                self.document.reporter.warning(
                    'Can\'t find default table style "{table_name}".  ' "Using built-in default.",
                    base_node=node,
                )
                table_style = BUILTIN_DEFAULT_TABLE_STYLE
        return table_style
        ...
        if isinstance(node.parent, docutils.nodes.section):
            ...
            if section_level > 7:
                self.document.reporter.warning(
                    ...
                    '    Reducing to heading level 7 for heading: "%s"' % (node.astext(),),
                    base_node=node,
                )
                section_level = 7
            el1 = self.append_child(
