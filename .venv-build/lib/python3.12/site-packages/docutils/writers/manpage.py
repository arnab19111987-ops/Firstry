...
class Translator(nodes.NodeVisitor):
    ...
    def header(self):
        th = ('.TH "%(title)s" "%(manual_section)s"' ' "%(date)s" "%(version)s"') % self._docinfo
    ...
    def visit_image(self, node):
        ...
        if "alt" in node.attributes:
            self.document.reporter.info(msg, base_node=node)
            self.body.append(".sp\n    %s\n" % (node.attributes["alt"]))
    ...
    def _visit_reference_no_macro(self, node) -> None:
        ...
        if "refuri" in node:
            ...
            if node["refuri"] == node.astext() or node["refuri"] == "mailto:" + node.astext():
                # without mailto:
                self.body.append(r"\%%<%s>" % insert_URI_breakpoints(node.astext()))
                raise nodes.SkipNode
