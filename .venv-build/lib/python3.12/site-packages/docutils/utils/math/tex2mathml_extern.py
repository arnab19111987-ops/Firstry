...
def _check_result(result, details=[]):
    ...
    if details:
        ...
        msg = (
            f"TeX to MathMl converter `{result.args[0]}` " f"exited with Errno {result.returncode}."
        )
...
def latexml(math_code, as_block=False):
    ...
    if result1.stderr:
        result1.stderr = "\n".join(
            line for line in result1.stderr.splitlines() if line.startswith(error_tags)
        )
    ...
    result2 = subprocess.run(args2, input=result1.stdout, capture_output=True, text=True)
    ...
    result2.stderr = "\n".join(
        line for line in _msg_source.splitlines() if line.startswith(error_tags)
    )
...
def pandoc(math_code, as_block=False):
    ...
    result.stdout = result.stdout[result.stdout.find("<math") : result.stdout.find("</math>") + 7]
...
def ttm(math_code, as_block=False):
    ...
    """
    args = ["ttm", "-L", "-r"]  # source is LaTeX snippet  # output MathML snippet
    ...
    result.stdout = result.stdout[result.stdout.find("<math") : result.stdout.find("</math>") + 7]
    if as_block:
        result.stdout = result.stdout.replace("<math xmlns=", '<math display="block" xmlns=')
    result.stderr = "\n".join(
        line[5:] + "." for line in result.stderr.splitlines() if line.startswith("**** ")
    )
    _check_result(result)
    return result.stdout
...
if __name__ == "__main__":
    example = (
        r"\frac{\partial \sin^2(\alpha)}{\partial \vec r}" r"\varpi \mathbb{R} \, \text{Grüße}"
    )
