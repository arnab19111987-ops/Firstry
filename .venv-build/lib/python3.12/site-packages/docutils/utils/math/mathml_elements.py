...
class MathElement(ET.Element):
    ...
    def __setitem__(self, key, value) -> None:
        ...
        if isinstance(value, MathElement):
            ...
            if self.nchildren and len(self) + len(value) > self.nchildren:
                raise TypeError(
                    f'Element "{self}" takes only {self.nchildren}' " children"
                )
            for e in value:
                e.parent = self
    ...
    def toxml(self, encoding=None):
        ...
        """
        xml = ET.tostring(self, encoding or "unicode", short_empty_elements=False)
...
class MathToken(MathElement):
    ...
    def __init__(self, text, **attributes) -> None:
        ...
        if not isinstance(text, (str, numbers.Number)):
            raise ValueError(
                "MathToken element expects `str` or number," f' not "{text}".'
            )
        self.text = str(text)
    ...
    def transfer_attributes(self, other) -> None:
        ...
        for k, v in self.items():
            if k in ("class", "style") and v:
                if other.get(k):
                    v = delimiters[k].join((other.get(k).rstrip(delimiters[k]), v))
            other.set(k, v)
