...
def extract_name_value(line):
    ...
    while line:
        ...
        if line[0] in "'\"":
            ...
            if endquote_index == -1:
                raise NameValueError(f'attribute "{attname}" missing end quote ({line[0]})')
            if len(line) > endquote_index + 1 and line[endquote_index + 1].strip():
                raise NameValueError(
                    f'attribute "{attname}" end quote ' f"({line[0]}) not followed by whitespace"
                )
...
def new_document(source_path: StrPath, settings: Values | None = None) -> nodes.document:
    """
