...
def _config_from_repository_url(url: str) -> RepositoryConfig:
    ...
    if parsed.username:
        ...
        config["repository"] = cast(str, rfc3986.urlparse(url).copy_with(userinfo=None).unsplit())
    config["repository"] = normalize_repository_url(cast(str, config["repository"]))
    return config
...
def make_requests_session() -> requests.Session:
    ...
    s.headers["User-Agent"] = (
        user_agent.UserAgentBuilder("twine", twine.__version__).include_implementation().build()
    )
    return s
