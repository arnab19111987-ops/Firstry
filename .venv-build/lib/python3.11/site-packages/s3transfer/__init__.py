...
def disable_upload_callbacks(request, operation_name, **kwargs):
    if operation_name in ["PutObject", "UploadPart"] and hasattr(request.body, "disable_callback"):
    ...
    def upload_file(self, filename, bucket, key, callback, extra_args):
        ...
        try:
            ...
            raise S3UploadFailedError(
                "Failed to upload {} to {}: {}".format(filename, "/".join([bucket, key]), e)
            )
        ...
        with self._os.open(filename, "wb") as f:
            while True:
                ...
                if task is SHUTDOWN_SENTINEL:
                    logger.debug(
                        "Shutdown sentinel received in IO handler, " "shutting down IO handler."
                    )
                    return
                else:
                    try:
...
class S3Transfer:
    ...
    def __init__(self, client, config=None, osutil=None):
        self._client = client
        self._client.meta.events.register("before-call.s3.*", self._update_checksum_context)
        if config is None:
            config = TransferConfig()
        self._config = config
        if osutil is None:
