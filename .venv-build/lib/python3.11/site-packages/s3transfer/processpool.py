    ...
    def _validate_all_known_args(self, provided):
        for kwarg in provided:
            if kwarg not in ALLOWED_DOWNLOAD_ARGS:
                ...
                raise ValueError(
                    f"Invalid extra_args key '{kwarg}', " f"must be one of: {download_args}"
                )
    ...
    def _do_run(self):
        while True:
            ...
            try:
                ...
                logger.debug(
                    "Exception caught when submitting jobs for " "download file request %s: %s",
    ...
    def _do_run(self):
        while True:
            ...
            if not self._transfer_monitor.get_exception(job.transfer_id):
                ...
                logger.debug(
                    "Skipping get object job %s because there was a previous " "exception.",
                    job,
                )
