    ...
    def _validate_all_known_args(self, actual, allowed):
        for kwarg in actual:
            if kwarg not in allowed:
                raise ValueError(
                    f"Invalid extra_args key '{kwarg}', " f"must be one of: {', '.join(allowed)}"
                )
...
class BotocoreCRTRequestSerializer(BaseCRTRequestSerializer):
    def __init__(self, session, client_kwargs=None):
        ...
        self._client.meta.events.register("before-call.s3.*", self._remove_checksum_context)
...
class S3ClientArgsCreator:
    ...
    def _get_make_request_args_put_object(
        ...
        if not any(
            checksum_arg in call_args.extra_args for checksum_arg in FULL_OBJECT_CHECKSUM_ARGS
        ):
    ...
    def _get_make_request_args_get_object(
        ...
        if isinstance(call_args.fileobj, str):
            ...
            on_done_before_calls.append(
                RenameTempFileHandler(coordinator, final_filepath, recv_filepath, self._os_utils)
            )
        else:
    ...
    def _default_get_make_request_args(
        ...
        make_request_args = {
            "request": self._request_serializer.serialize_http_request(request_type, future),
