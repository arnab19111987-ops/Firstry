...
class UploadSubmissionTask(SubmissionTask):
    ...
    def _submit_upload_request(
        ...
        put_object_extra_args = self._extra_put_object_args(call_args.extra_args)
        ...
        self._transfer_coordinator.submit(
            ...
            PutObjectTask(
                ...
                main_kwargs={
                    "client": client,
                    "fileobj": upload_input_manager.get_put_object_body(transfer_future),
    ...
    def _submit_multipart_request(
        ...
        for checksum in FULL_OBJECT_CHECKSUM_ARGS:
            if checksum in call_args.extra_args:
                ...
                call_args.extra_args["ChecksumAlgorithm"] = checksum.replace("Checksum", "")

        create_multipart_extra_args = self._extra_create_multipart_args(call_args.extra_args)
    ...
    def _extra_create_multipart_args(self, extra_args):
        return get_filtered_dict(extra_args, blocklisted_keys=self.CREATE_MULTIPART_BLOCKLIST)

    def _extra_put_object_args(self, extra_args):
        return get_filtered_dict(extra_args, blocklisted_keys=self.PUT_OBJECT_BLOCKLIST)
