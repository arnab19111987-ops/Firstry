...
class ResponseParser:
    ...
    def __init__(self, timestamp_parser=None, blob_parser=None):
        ...
        if self.EVENT_STREAM_PARSER_CLS is not None:
            self._event_stream_parser = self.EVENT_STREAM_PARSER_CLS(timestamp_parser, blob_parser)
    ...
    def _has_unknown_tagged_union_member(self, shape, value):
        if shape.is_tagged_union:
            ...
            serialized_member_names = [
                shape.members[member].serialization.get("name", member) for member in shape.members
            ]
...
class BaseXMLResponseParser(ResponseParser):
    ...
    def _handle_structure(self, shape, node):
        ...
        for member_name in members:
            ...
            if location in self.KNOWN_LOCATIONS or member_shape.serialization.get("eventheader"):
...
class BaseCBORParser(ResponseParser):
    ...
    def _parse_tag(self, stream, additional_info):
        ...
        if tag == 1:  # Epoch-based date/time in milliseconds
            ...
            raise ResponseParserError(f"Found CBOR tag not supported by botocore: {tag}")
...
class EventStreamCBORParser(BaseEventStreamParser, BaseCBORParser):
    def _initial_body_parse(self, body_contents):
        if body_contents == b"":
            return {}
        return self.parse_data_item(self.get_peekable_stream_from_bytes(body_contents))
    ...
    def _do_parse(self, response, shape):
        ...
        if shape is not None:
            ...
            if event_stream_name:
                parsed = self._handle_event_stream(response, shape, event_stream_name)
            else:
                parsed = {}
                self._parse_payload(response, shape, parsed)
            parsed["ResponseMetadata"] = self._populate_response_metadata(response)
        return parsed
...
class RpcV2CBORParser(BaseRpcV2Parser, BaseCBORParser):
    ...
    def _do_error_parse(self, response, shape):
        ...
        if code is not None:
            ...
            error["Error"]["Code"] = code
        if "x-amzn-requestid" in headers:
            error.setdefault("ResponseMetadata", {})["RequestId"] = headers["x-amzn-requestid"]
        return error
