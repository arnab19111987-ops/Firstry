...
class EndpointResolver(BaseEndpointResolver):
    ...
    def _endpoint_for_partition(
        ...
        if use_dualstack_endpoint and partition_name in self._UNSUPPORTED_DUALSTACK_PARTITIONS:
            error_msg = (
                "Dualstack endpoints are currently not supported" f" for {partition_name} partition"
            )
    ...
    def _resolve(
        ...
        if tags:
            ...
            if result == {}:
                error_msg = (
                    f"Endpoint does not exist for {service_name} " f"in region {endpoint_name}"
                )
    ...
    def _expand_template(self, partition, template, service_name, endpoint_name, dnsSuffix):
        return template.format(service=service_name, region=endpoint_name, dnsSuffix=dnsSuffix)
...
class EndpointRulesetResolver:
    ...
    def _resolve_param_as_operation_context_param(self, param_name, operation_model, call_args):
    ...
    def auth_schemes_to_signing_ctx(self, auth_schemes):
        ...
        auth_schemes = [
            {**scheme, "name": self._strip_sig_prefix(scheme["name"])} for scheme in auth_schemes
        ]
            ...
            try:
                ...
                if not HAS_CRT:
                    fixable_with_crt = any(
                        scheme in CRT_SUPPORTED_AUTH_TYPES for scheme in auth_type_options
                    )
