...
class CBORSerializer(Serializer):
    ...
    def _serialize_data_item(self, serialized, value, shape, key=None):
        ...
        if method is None:
            raise ValueError(
                f"Unrecognized C2J type: {shape.type_name}, unable to " f"serialize request"
            )
        method(serialized, value, shape, key)
    ...
    def _partition_parameters(self, partitioned, param_name, param_value, shape_members):
        ...
        if location == "uri":
            ...
            if not param_value and shape.type_name == "list":
                ...
                return
            partitioned["headers"][key_name] = self._convert_header_value(shape, param_value)
    ...
    def _convert_header_value(self, shape, value):
        if shape.type_name == "timestamp":
            ...
            if shape.member.type_name == "string":
                ...
                converted_value = [
                    self._convert_header_value(shape.member, v) for v in value if v is not None
                ]
...
class RestXMLSerializer(BaseRestSerializer):
    ...
    def _serialize_type_timestamp(self, xmlnode, params, shape, name):
        node = ElementTree.SubElement(xmlnode, name)
        node.text = str(
            self._convert_timestamp_to_str(params, shape.serialization.get("timestampFormat"))
        )
        self._add_xml_namespace(shape, node)
