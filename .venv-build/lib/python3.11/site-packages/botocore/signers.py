    ...
    * before-sign: Allows mutating the request before signing.
        ...
        if signature_version != botocore.UNSIGNED:
            ...
            try:
                ...
                if signing_type != "standard":
                    raise UnsupportedSignatureVersionError(signature_version=signature_version)
                else:
                    raise e
    ...
    def _choose_signer(self, operation_name, signing_type, context):
        ...
        if response is not None:
            ...
            if signature_version is not botocore.UNSIGNED and not signature_version.endswith(
                suffix
            ):
                signature_version += suffix
...
def add_dsql_generate_db_auth_token_methods(class_attributes, **kwargs):
    class_attributes["generate_db_connect_auth_token"] = dsql_generate_db_connect_auth_token
...
def dsql_generate_db_connect_admin_auth_token(self, Hostname, Region=None, ExpiresIn=900):
    ...
    :return: A presigned url which can be used as an auth token.
    """
    return _dsql_generate_db_auth_token(self, Hostname, "DbConnectAdmin", Region, ExpiresIn)
