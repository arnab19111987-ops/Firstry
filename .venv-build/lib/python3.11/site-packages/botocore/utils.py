...
class InstanceMetadataFetcher(IMDSFetcher):
    ...
    def _evaluate_expiration(self, credentials):
        ...
        try:
            ...
            logger.debug("Unable to parse expiry_time in %s", credentials["expiry_time"])
...
class IMDSRegionProvider:
    ...
    def _create_fetcher(self):
        ...
        imds_config = {
            ...
            "ec2_metadata_service_endpoint_mode": resolve_imds_endpoint_mode(self._session),
...
def parse_timestamp(value):
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                'Unable to parse timestamp using fallback method with "%s" ' "timezone info.",
                tzinfo.__name__,
                exc_info=e,
            )
...
def switch_to_virtual_host_style(request, signature_version, default_endpoint_url=None, **kwargs):
    ...
    if request.auth_path is not None:
        ...
        # issues.
        logger.debug(
            "Request is GetBucketLocation operation, not checking " "for DNS compatibility."
        )
        return
        ...
        if signing_name == "s3express" and signature_version.startswith("v4-s3express"):
            ...
            if "cache_key" not in signing_context:
                signing_context["cache_key"] = (
                    request.context.get("s3_redirect", {}).get("params", {}).get("Bucket")
                )
...
class S3RegionRedirectorv2:
    ...
    def redirect_from_error(self, request_dict, response, operation, **kwargs):
        ...
        is_opt_in_region_redirect = (
            error_code == "IllegalLocationConstraintException" and operation.name != "CreateBucket"
        )
        if not any(
            [
...
class S3EndpointSetter:
    ...
    def _validate_mrap_s3_config(self, request):
        ...
        if self._s3_config.get("s3_disable_multiregion_access_points"):
            raise UnsupportedS3AccesspointConfigurationError(
                msg=("Invalid configuration, Multi-Region Access Point " "ARNs are disabled.")
            )
...
class S3ControlArnParamHandler:
    ...
    def _override_account_id_param(self, params, arn_details):
        ...
        if "AccountId" in params and params["AccountId"] != account_id:
            error_msg = (
                "Account ID in arn does not match the AccountId parameter " 'provided: "{}"'
...
class ContainerMetadataFetcher:
    ...
    def _get_response(self, full_url, headers, timeout):
        try:
            ...
            error_msg = "Received error when attempting to retrieve " f"container metadata: {e}"
            raise MetadataRetrievalError(error_msg=error_msg)
...
def is_s3_accelerate_url(url):
    ...
    if not url_parts.netloc.endswith("amazonaws.com") or url_parts.scheme not in ["https", "http"]:
        return False
