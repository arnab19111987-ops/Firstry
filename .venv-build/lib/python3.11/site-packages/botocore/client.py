...
class ClientCreator:
    ...
    def create_client(
        ...
        if token := self._evaluate_client_specific_token(service_model.signing_name):
            auth_token = token
    ...
    def _register_s3_events(
        ...
        client.meta.events.register("before-parameter-build.s3", self._inject_s3_input_parameters)
    ...
    def _register_importexport_events(
        ...
        if client.meta.service_model.service_name != "importexport":
            return
        self._set_importexport_signature_version(client.meta, client_config, scoped_config)

    def _set_importexport_signature_version(self, client_meta, client_config, scoped_config):
        # This will return the manually configured signature version, or None
        # if none was manually set. If a customer manually sets the signature
        # version, we always want to use what they set.
        ...
        client_meta.events.register("choose-signer.importexport", self._default_signer_to_sigv4)
...
class BaseClient:
    ...
    def __init__(
        ...
        if self._user_agent_creator is None:
            self._user_agent_creator = UserAgentString.from_environment().with_client_config(
                self._client_config
            )
    ...
    def __getattr__(self, item):
        ...
        handler, event_response = self.meta.events.emit_until_response(event_name, client=self)
    ...
    def _register_handlers(self):
        ...
        self.meta.events.register(f"request-created.{service_id}", self._request_signer.handler)
    ...
    def _make_api_call(self, operation_name, api_params):
        ...
        if http.status_code >= 300:
            ...
            error_code = request_context.get("error_code_override") or error_info.get("Code")
