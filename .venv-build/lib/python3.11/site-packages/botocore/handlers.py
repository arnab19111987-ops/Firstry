...
def decode_console_output(parsed, **kwargs):
    if "Output" in parsed:
        try:
            ...
            value = base64.b64decode(bytes(parsed["Output"], "latin-1")).decode("utf-8", "replace")
            parsed["Output"] = value
...
def add_retry_headers(request, **kwargs):
    ...
    sdk_request_headers = [
        f"{key}={retries_context[key]}" for key in sdk_retry_keys if key in retries_context
    ]
...
def remove_content_type_header_for_presigning(request, **kwargs):
    if request.context.get("is_presign_request") is True and "Content-Type" in request.headers:
...
def _has_expires_shape(shape):
    if not shape:
        return False
    return any(
        member_shape.name == "Expires" and member_shape.serialization.get("name") == "Expires"
...
def document_expires_shape(section, event_name, **kwargs):
    ...
    if "response-example" in event_name:
        ...
        doc_section.write("This member has been deprecated. Please use ``ExpiresString`` instead.")
...
def _handle_200_error(operation_model, response_dict, **kwargs):
    ...
    if not _should_handle_200_error(operation_model, response_dict):
        ...
        return
    if _looks_like_special_case_error(response_dict["status_code"], response_dict["body"]):
...
def _set_auth_scheme_preference_signer(context, signing_name, **kwargs):
    ...
    signature_version_set_in_code = (
        isinstance(signature_version, ClientConfigString) or signature_version is botocore.UNSIGNED
    )
    auth_preference_set_in_code = isinstance(auth_scheme_preference, ClientConfigString)
    has_in_code_configuration = signature_version_set_in_code or auth_preference_set_in_code
    ...
    if not signature_version_set_in_code and auth_scheme_preference and auth_options:
        ...
        resolved = botocore.auth.resolve_auth_scheme_preference(preferred_schemes, auth_options)
        resolved_signature_version = botocore.UNSIGNED if resolved == "none" else resolved
...
def _should_prefer_bearer_auth(
    ...
    return has_token and (resolved_signature_version == "bearer" or not has_in_code_configuration)
