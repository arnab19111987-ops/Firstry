...
class ClientArgsCreator:
    ...
    def get_client_args(
        ...
        serializer_kwargs = {"timestamp_precision": botocore.serialize.TIMESTAMP_PRECISION_DEFAULT}
        event_emitter.emit(
            event_name,
    ...
    def compute_client_args(
        ...
        if client_config is not None:
            config_kwargs.update(
                ...
                request_checksum_calculation=(client_config.request_checksum_calculation),
        ...
        return {
            ...
            "s3_config": s3_config,
            "socket_options": self._compute_socket_options(scoped_config, client_config),
        }
    ...
    def _compute_inject_host_prefix(self, client_config, config_kwargs):
        ...
        if client_config is None or client_config._inject_host_prefix == "UNSET":
            configured_disable_host_prefix_injection = self._config_store.get_config_variable(
                "disable_host_prefix_injection"
            )
            if configured_disable_host_prefix_injection is not None:
                config_kwargs["inject_host_prefix"] = not configured_disable_host_prefix_injection
            else:
    ...
    def _resolve_endpoint(
        ...
        return endpoint_bridge.resolve(service_name, region_name, endpoint_url, is_secure)
    ...
    def _validate_min_compression_size(self, min_size):
        ...
        try:
            ...
            msg = (
                f"{error_msg_base} Value must be an integer. " f"Received {type(min_size)} instead."
            )
    ...
    def compute_endpoint_resolver_builtin_defaults(
        ...
        return {
            ...
            EPRBuiltins.AWS_USE_FIPS: (
                ...
                and endpoint_bridge._resolve_endpoint_variant_config_var("use_fips_endpoint")
                or False
            ),
            ...
            EPRBuiltins.AWS_S3_ACCELERATE: s3_config.get("use_accelerate_endpoint", False),
            EPRBuiltins.AWS_S3_FORCE_PATH_STYLE: force_path_style,
            EPRBuiltins.AWS_S3_USE_ARN_REGION: s3_config.get("use_arn_region", True),
            EPRBuiltins.AWS_S3CONTROL_USE_ARN_REGION: s3_config.get("use_arn_region", False),
    ...
    def _compute_account_id_endpoint_mode_config(self, config_kwargs):
        ...
        if account_id_endpoint_mode is None:
            account_id_endpoint_mode = self._config_store.get_config_variable(config_key)
    ...
    def _compute_auth_scheme_preference_config(self, client_config, config_kwargs):
        ...
        value = ",".join(
            item.replace(" ", "").replace("\t", "") for item in value.split(",") if item.strip()
        )
