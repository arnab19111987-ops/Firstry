...
def detect_js_project_type(repo_root: str) -> Dict[str, Any]:
    ...
    deps = {**package_json_info.get("dependencies", {}), **package_json_info.get("devDependencies", {})}
    test_frameworks = []
...
def get_js_related_files(repo_root: str, changed_files: List[str] = None) -> Set[str]:
    ...
    if changed_files:
        ...
        for file_path in changed_files:
            ...
            if any(
                filename == pattern or 
                (pattern.endswith("*") and filename.startswith(pattern[:-1]))
        ...
        js_dirs = ["src", "lib", "app", "pages", "components", "test", "tests", "__tests__"]
        
        for js_dir in js_dirs:
            dir_path = repo_path / js_dir
            if dir_path.exists():
                for ext in js_extensions:
                    js_files.update(str(f.relative_to(repo_path)) for f in dir_path.rglob(f"*{ext}"))
    ...
    try:
        ...
        if use_cache and success and js_files:
            ...
            ft_cache.write_tool_cache(
                repo_root, "npm_test", input_hash, "ok",
                {
                    "duration": duration,
...
def analyze_npm_project(repo_root: str) -> Dict[str, Any]:
    ...
    if project_info["test_frameworks"]:
        if "cypress" in project_info["test_frameworks"] or "playwright" in project_info["test_frameworks"]:
            test_complexity = "high"  # E2E tests
            estimated_duration = "60-180s"
        elif "jest" in project_info["test_frameworks"] or "vitest" in project_info["test_frameworks"]:
