...
def run_tool_with_smart_cache(repo_root: str, tool_name: str, input_paths: List[str]) -> Dict[str, Any]:
    """
...
def _tool_to_cmd(tool_name: str) -> List[str]:
    ...
    command_map = {
        "ruff": ["ruff", "check", "."],
        "repo_sanity": ["python", "-c", "print('Repo sanity check passed')"],  # placeholder
        "black": ["black", "--check", "."],
        "mypy": ["mypy", "."],
        "pytest": ["python", "-m", "pytest", "-q"],  # Basic fallback - smart pytest handles this
        "npm test": ["npm", "test"],
...
async def run_checks_for_profile(
    ...
    progress.step(f"Running {len(checks)} checks with caching={'enabled' if use_cache else 'disabled'}")
        ...
        for chk, inp_hash, coro in fast_tasks:
            ...
            try:
                ...
                if rc == 0:
                    ...
                    ft_cache.write_tool_cache(repo_root, chk, inp_hash, "fail", {"elapsed": elapsed, "output": out})
    ...
    if mut_checks:
        ...
        for chk in mut_checks:
            ...
            if use_cache:
                ...
                if is_valid:
                    progress.cached(chk)
                    results[chk] = {"status": "ok", "cached": True, "cache_state": cache_state}
                    continue
    ...
    progress.step(f"⏱  Total active check time (not counting cache hits): {total_elapsed:.2f}s")
