...
def _group_impacted_by_project(twin: CodebaseTwin, changed: List[str]) -> Dict[str, Set[str]]:
...
def build_plan_from_twin(
    ...
    *,
    ...
    for proj_name, files in per_proj_impacted.items():
        lang = (twin.projects.get(proj_name).lang if proj_name in twin.projects else "python")
        ...
        if lang == "python":
            ...
            if py_changed:
                ...
                add(Task(id=pt_id, check_id="pytest", targets=test_targets or [root], flags=["-q"], deps=deps))
            ...
            js_changed = any(f.endswith(suf) for f in files for suf in (".js",".jsx",".ts",".tsx")) or proj_name == "_root"
            if js_changed:
                lint_id = f"npm-lint:{proj_name}"
                test_id = f"npm-test:{proj_name}"
                add(Task(id=lint_id, check_id="npm-lint", targets=[root], flags=[]))
                deps = {f"{d}:{proj_name}" for d in workflow_requires if d in {"npm-lint"}}
                add(Task(id=test_id, check_id="npm-test", targets=[root], flags=[], deps=deps))
