...
def _print_help() -> None:
    ...
    print(f"Core fast flows:")
    print(f"  {prog} lite -> python -m firsttry run fast --tier free-lite --profile fast --report-json .firsttry/report.json")
    print(f"  {prog} strict -> python -m firsttry run strict --tier free-strict --report-json .firsttry/report.json")
    print(f"  {prog} pro -> python -m firsttry run fast --tier pro --report-json .firsttry/report.json")
    print(f"\nMaintenance / visibility:")
    print(f"  {prog} doctor -> python -m firsttry doctor")
    print(f"  {prog} doctor-checks -> python -m firsttry doctor --check report-json --check telemetry")
    print(f"  {prog} setup -> python -m firsttry setup --install-hooks")
    print(f"  {prog} dash -> python -m firsttry inspect dashboard --json .firsttry/report.json")
    print(f"  {prog} lock -> python -m firsttry inspect report --json .firsttry/report.json --filter locked=true")
