...
async def run_comprehensive_benchmark(repo_root: str) -> PerformanceBenchmark:
    ...
    if "Full Suite (Optimized)" in results_by_scenario and "Incremental (Python Changes)" in results_by_scenario:
        metrics = benchmark.calculate_metrics(
...
async def validate_performance_targets(benchmark: PerformanceBenchmark) -> Dict[str, bool]:
