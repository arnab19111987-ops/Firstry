...
def _explain_security_groups(result: ScanResult) -> list[str]:
    ...
    if high_unrev and high_unrev > 0:
        lines.append(
            "  → You must review and fix/annotate those high-risk files "
            "before you push."
        )
    else:
...
def _overall_gate_message(result: ScanResult, gate_name: str) -> str:
    ...
    if gate_name == "pre-commit":
        ...
        if autofix_block:
            return (
                "❌  Not ready. Fix autofixable issues (see commands below) "
                "before committing."
            )
    ...
    if has_failed_section:
        ...
        return (
            "❌  Not ready. One or more sections are still failing. "
...
def _autofix_hints(sections: Iterable[SectionResult]) -> list[str]:
    ...
    for s in sections:
        ...
        if not (
            getattr(s, "autofixable_count", 0) > 0 or getattr(s, "manual_count", 0) > 0
        ):
            continue
