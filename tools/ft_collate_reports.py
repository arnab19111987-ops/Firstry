    ...
    for tier, phases in tiers.items():
        ...
        for ph in ("cold", "warm"):
            ...
            if ds:
                ...
                phases[ph]["p95_ms"] = (
                    int(statistics.quantiles(ds, n=20)[18]) if len(ds) > 5 else max(ds)
                )
            else:
                phases[ph]["p50_ms"] = phases[ph]["p95_ms"] = None
        # detect regressions: warm slower than cold by >25%
        wc = phases.get("warm", {}).get("p50_ms")
        cc = phases.get("cold", {}).get("p50_ms")
...
def write_outputs(tiers):
    ...
    for tier, ph in sorted(tiers.items()):
        ...
        w = ph.get("warm", {})
        lines.append(
            f"- Cold: p50={c.get('p50_ms')}ms, p95={c.get('p95_ms')}ms, ok={c.get('ok', 0)}/{c.get('count', 0)}",
        )
        lines.append(
            f"- Warm: p50={w.get('p50_ms')}ms, p95={w.get('p95_ms')}ms, ok={w.get('ok', 0)}/{w.get('count', 0)}, cache_hit_rate={w.get('cache_hit_rate')}",
        )
        if ph.get("regression_flag"):
            lines.append("- ⚠️ Warm slower than cold (>25%)")
        # slowest 3 warm checks
        checks = w.get("checks", {})
        slow = sorted(checks.items(), key=lambda kv: kv[1].get("duration_ms") or 0, reverse=True)[
            :3
        ]
        if slow:
...
def main():
    items = load_reports()
    if not items:
        print(f"No reports in {SRC} — run scripts/ft_tier_sweep.sh first.")
        sys.exit(1)
