...
def parse_manual_result(name: str):
    ...
    if name == "manual_ruff":
        ...
        try:
            ...
            errs = len(re.findall(r'^[A-Z].+:\d+:\d+:', out, flags=re.M))
...
def ft_check(checks: dict, key_prefix: str):
    ...
    for k, v in checks.items():
        if k.startswith(key_prefix + ":") or k == key_prefix or v.get("meta", {}).get("check_id") == key_prefix:
...
def errors_from_text(tool: str, text: str):
    ...
    if tool == "ruff":
        return len(re.findall(r'^[A-Z].+:\d+:\d+:', text, flags=re.M))
    if tool == "mypy":
...
def main():
    ...
    lines.append("| Tool | Manual (s) | Manual errors | FT (ms, warm) | FT status | Cache | FT errors | Speedup× | Parity |")
    lines.append("|---|---:|---:|---:|:--:|:--:|---:|---:|:--:|")
    for r in rows:
        lines.append(f"| {r['tool']} | {r.get('manual_s') if r.get('manual_s') is not None else ''} "
