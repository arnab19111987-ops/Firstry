...
def test_find_unmapped_steps_detects_unmapped_action(tmp_path):
    ...
    unmapped = find_unmapped_steps(mirror_path=str(mirror), workflows_root=str(workflows_root))
...
def test_missing_job_reports_missing_mapping(tmp_path):
    ...
    unmapped = find_unmapped_steps(mirror_path=str(mirror), workflows_root=str(workflows_root))
    assert any(u.job == "unmapped_job" and "missing mirror mapping" in u.reason for u in unmapped)
...
def test_get_mirror_status_marks_missing_and_extra(tmp_path):
    ...
    mirror_dir.mkdir()
    mirror = mirror_dir / "ci_mirror.toml"
    mirror.write_text(textwrap.dedent('''
      [[jobs]]
      workflow = "absent.yml"
      job_id = "a"
    '''))
    ...
    assert "present.yml" in ms.missing_workflows or isinstance(ms.missing_workflows, list)
